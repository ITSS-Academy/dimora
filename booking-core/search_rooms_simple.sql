-- RPC Function đơn giản cho search rooms (không cần PostGIS)
-- Sử dụng công thức Haversine để tính khoảng cách

-- Function tính khoảng cách giữa 2 điểm (km)
...
-- RPC Function search rooms đơn giản (không cần PostGIS)
CREATE OR REPLACE FUNCTION search_rooms_simple(
  lat NUMERIC,
  lng NUMERIC,
...
BEGIN
  SELECT json_agg(
    json_build_object(
      ...
      'distance_km', calculate_distance_km(lat, lng, r.latitude, r.longitude)
    )
  ) INTO result
  FROM rooms r
  WHERE 
    ...
    AND calculate_distance_km(lat, lng, r.latitude, r.longitude) <= radius_km
  ...
  ORDER BY calculate_distance_km(lat, lng, r.latitude, r.longitude) ASC;
...
GRANT EXECUTE ON FUNCTION search_rooms_simple(NUMERIC, NUMERIC, DATE, DATE, INTEGER, INTEGER, NUMERIC, NUMERIC) TO authenticated;

-- Test function
SELECT * FROM search_rooms_simple(
  lat := 10.762622,
  lng := 106.660172,
  radius_km := 10
);
