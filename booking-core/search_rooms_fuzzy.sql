    ...
    json_build_object(
      ...
      'search_score', 
        CASE 
          ...
          WHEN lower(r.location) = lower(search_term) THEN 95
          ...
          WHEN normalize_vietnamese_text(r.city) = normalized_search THEN 85
          WHEN normalize_vietnamese_text(r.location) = normalized_search THEN 82
          WHEN normalize_vietnamese_text(r.address) = normalized_search THEN 80
          
          -- Contains matches (75-65)
          WHEN lower(r.city) LIKE '%' || lower(search_term) || '%' THEN 75
          WHEN lower(r.location) LIKE '%' || lower(search_term) || '%' THEN 70
          WHEN lower(r.address) LIKE '%' || lower(search_term) || '%' THEN 65
          WHEN lower(r.title) LIKE '%' || lower(search_term) || '%' THEN 60
          WHEN lower(r.description) LIKE '%' || lower(search_term) || '%' THEN 55
          
          -- Normalized contains (50-40)
          WHEN normalize_vietnamese_text(r.city) LIKE '%' || normalized_search || '%' THEN 50
          WHEN normalize_vietnamese_text(r.location) LIKE '%' || normalized_search || '%' THEN 45
          WHEN normalize_vietnamese_text(r.address) LIKE '%' || normalized_search || '%' THEN 40
          
          -- Fuzzy matches (35-20)
          WHEN calculate_string_similarity(r.city, search_term) >= 60 THEN 35
          WHEN calculate_string_similarity(r.location, search_term) >= 60 THEN 30
          WHEN calculate_string_similarity(r.address, search_term) >= 60 THEN 25
          WHEN calculate_string_similarity(r.title, search_term) >= 60 THEN 20
          ...
          WHEN calculate_string_similarity(r.city, search_term) >= 40 THEN 8
          WHEN calculate_string_similarity(r.location, search_term) >= 40 THEN 6
          WHEN calculate_string_similarity(r.address, search_term) >= 40 THEN 5
      ...
      CASE 
        WHEN search_term IS NOT NULL THEN
          CASE 
            ...
            -- Fuzzy
            WHEN calculate_string_similarity(r.city, search_term) >= 60 THEN 35
            WHEN calculate_string_similarity(r.location, search_term) THEN 30
            WHEN calculate_string_similarity(r.address, search_term) >= 60 THEN 25
            WHEN calculate_string_similarity(r.title, search_term) >= 60 THEN 20
