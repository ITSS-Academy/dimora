-- RPC Functions đã sửa lỗi ambiguous column
-- Sử dụng parameter names khác với column names
...
BEGIN
  ...
  LEFT JOIN users u ON b.user_id = u.id
  WHERE b.host_id = host_uuid
    AND b.check_in_date >= start_date_param
    AND b.check_out_date <= end_date_param
  ORDER BY b.check_in_date ASC;
...
-- 5. Function để check room availability nhanh (đã sửa lỗi ambiguous)
...
BEGIN
  ...
  FROM (
    SELECT generate_series(start_date_param, end_date_param, '1 day'::interval)::date as date_val
  ) date_series;
