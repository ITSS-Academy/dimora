...
@Component({
  ...
  standalone: true,
  imports: [MaterialModule, ReactiveFormsModule, NgIf, LoadingComponent, AsyncPipe],
...
export class BookingComponent implements OnInit, OnDestroy {
    ...
    this.subscription.push(
      ...
      this.errorBooking$.subscribe(error => {
        if(error){
          ...
          this.snackbarService.showAlert(error.error.message, 'error', 3000, 'right','top');
        }
      })
  ...
  onSubmit(): void {
    if (this.bookingForm.valid) {
      ...
      this.store.dispatch(BookingActions.createBooking({ booking: bookingData, idToken: this.idToken }));
